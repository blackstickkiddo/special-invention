...
let totalCandies = bulbasaurCadies + caterpieCandies + weedleCandies
...
function newTemp(Temp) {
    let farenheitToCelcius = (Temp - 32) * 5 / 9
    return farenheitToCelcius
}

function canCharmanderBattle(tempNow) {
    let otherTemp = newTemp(tempNow)
    if (otherTemp > 0) {
        console.log('Charmander can battle')
    } else {
        console.log('It is too cold for Charmander')
    }
}
...
function pokemonFightClub(){
    for (let i = 0; i <= numberofFighters.length; i++){
        console.log('Pikachu I choose you')
    }
}

let numberofFighters = [('idk1', 'idk2', 'idk3', 'idk4')]
pokemonFightClub()
